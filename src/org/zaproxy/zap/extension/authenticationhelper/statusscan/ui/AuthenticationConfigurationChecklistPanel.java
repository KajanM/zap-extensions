...
public class AuthenticationConfigurationChecklistPanel extends JPanel {
	...
	/**
	 * Constant indicating the authentication <strong>configuration</strong> status.
	 * This can be one of {@code GOOD,
	 * BAD, MINIMAL, NOT_VALIDATED_YET}.
	 * <p>
	 * This should be differentiated from {@link AuthenticationStatus} which
	 * indicates the <strong>authentication</strong> status, based on the presence
	 * of logged in/out indicator in the response message.
	 */
	...
	static {
		GOOD_CONFIGURATION_ICON = new ImageIcon(AuthenticationConfigurationChecklistPanel.class
				.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/help/contents/images/tick-circle.png"));
		NOT_VALIDATED_YET_ICON = new ImageIcon(AuthenticationConfigurationChecklistPanel.class
				.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/help/contents/images/question-white.png"));
		BAD_CONFIGURATION_ICON = new ImageIcon(AuthenticationConfigurationChecklistPanel.class
				.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/help/contents/images/cross-circle.png"));
		MINIMAL_CONFIGURATION_ICON = new ImageIcon(AuthenticationConfigurationChecklistPanel.class
				.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/help/contents/images/exclamation-circle.png"));
	}
	...
	public AuthenticationConfigurationChecklistPanel(AuthenticationHelperDialog helperDialog) {
		...
		labelChecklistItemTarget = new JLabel(
				Constant.messages.getString("authenticationhelper.checklist.label.target"), NOT_VALIDATED_YET_ICON,
				JLabel.LEFT);
		labelChecklistItemContext = new JLabel(
				Constant.messages.getString("authenticationhelper.checklist.label.context"), NOT_VALIDATED_YET_ICON,
				JLabel.LEFT);
		labelChecklistItemUser = new JLabel(Constant.messages.getString("authenticationhelper.checklist.label.user"),
				NOT_VALIDATED_YET_ICON, JLabel.LEFT);
		labelChecklistItemAuthenticationMethod = new JLabel(
				Constant.messages.getString("authenticationhelper.checklist.label.authentication"),
				NOT_VALIDATED_YET_ICON, JLabel.LEFT);
		labelChecklistItemIndicator = new JLabel(
		...
		checklistStatusMap.put(labelChecklistItemAuthenticationMethod, ConfigurationStatus.NOT_VALIDATED_YET);
	 ...
	 * if {@code true} {@code (labelChecklistItemTarget, ConfigurationStatus.GOOD)}
	 * is put into {@code checklistStatusMap}. <br>
	 * if {@code false} {@code (labelChecklistItemTarget, ConfigurationStatus.BAD)}
	 * is put into {@code checklistStatusMap}
	 */
	private void checkTargetAndUpdateMap() {
		//@formatter:off
		boolean isBadConfiguration = helperDialog.getTarget() == null ||
									 helperDialog.getTarget().getStartNode() == null;
		//@formatter:on
	 ...
	 * if {@code true} {@code (labelChecklistItemContext, ConfigurationStatus.GOOD)}
	 * is put into {@code checklistStatusMap}. <br>
	 * if {@code false} {@code (labelChecklistItemContext, ConfigurationStatus.BAD)}
	 * is put into {@code checklistStatusMap}
	 */
	private void checkContextAndUpdateMap() {
		//@formatter:off
		boolean isBadConfiguration = !checklistStatusMap.get(labelChecklistItemTarget).equals(ConfigurationStatus.GOOD) ||
	...
	/**
	 * Checks if {@link AuthenticationMethod} is configured correctly for the
	 * selected {@link Context}. <br>
	 * if {@code true}
	 * {@code (labelChecklistItemAuthenticationMethod, ConfigurationStatus.GOOD)} is
	 * put into {@code checklistStatusMap}. <br>
	 * if {@code false}
	 * {@code (labelChecklistItemAuthenticationMethod, ConfigurationStatus.BAD)} is
	...
	private void checkAuthenticationMethodAndUpdateMap() {
		...
		//@formatter:off
		boolean isBadConfiguration = !checklistStatusMap.get(labelChecklistItemTarget).equals(ConfigurationStatus.GOOD) ||
				!checklistStatusMap.get(labelChecklistItemContext).equals(ConfigurationStatus.GOOD) ||
	...
	/**
	 * Checks if a {@link User} is configured correctly for the selected
	 * {@link Context}. <br>
	 * if {@code true} {@code (labelChecklistItemUser, ConfigurationStatus.GOOD)} is
	 * put into {@code checklistStatusMap}. <br>
	 * if {@code false} {@code (labelChecklistItemUser, ConfigurationStatus.BAD)} is
	 * put into {@code checklistStatusMap}
	 */
	private void checkUserAndUpdateMap() {
		...
		boolean isBadConfiguration = !checklistStatusMap.get(labelChecklistItemTarget).equals(ConfigurationStatus.GOOD) ||
				!checklistStatusMap.get(labelChecklistItemContext).equals(ConfigurationStatus.GOOD) ||
				!checklistStatusMap.get(labelChecklistItemAuthenticationMethod).equals(ConfigurationStatus.GOOD) ||
				selectedUser == null ||
	...
	/**
	 * Checks if {@code loggedInIndicator} and {@code loggedOutIndicator} are
	 * defined for the selected {@link Context}.<br>
	 * if both are defined
	 * {@code (labelChecklistItemIndicator, ConfigurationStatus.GOOD)} is put into
	 * {@code checklistStatusMap}. <br>
	 * if only one of them is defined
	 * {@code (labelChecklistItemIndicator, ConfigurationStatus.MINIMAL)} is put
	 * into {@code checklistStatusMap} if none is defined
	...
	private void checkIndicatorAndUpdateMap() throws IllegalArgumentException {
		...
		boolean isBadPreCondition = !checklistStatusMap.get(labelChecklistItemTarget).equals(ConfigurationStatus.GOOD)
				|| !checklistStatusMap.get(labelChecklistItemContext).equals(ConfigurationStatus.GOOD);

		if (isBadPreCondition) {
			status = ConfigurationStatus.BAD;
		} else {
			boolean inDefined = indicatorDefined(getConfiguredAuthenticationMethod().getLoggedInIndicatorPattern());
			boolean outDefined = indicatorDefined(getConfiguredAuthenticationMethod().getLoggedOutIndicatorPattern());
	...
	private void determineOverallStatusAndSetupHintPanel() {
		...
		if (!checklistStatusMap.get(labelChecklistItemAuthenticationMethod).equals(ConfigurationStatus.GOOD)) {
			overallConfigurationStatus = ConfigurationStatus.BAD;
	...
	private void setHintPanelToMatchBadTarget() {
		labelHint.setText(Constant.messages.getString("authenticationhelper.checklist.hint.target"));
		btnSettings.setEnabled(false);
	}

	private void setHintPanelToMatchBadContext() {
		labelHint.setText(Constant.messages.getString("authenticationhelper.checklist.hint.context"));
		btnSettings.setEnabled(true);
		btnSettings.setToolTipText(Constant.messages.getString("authenticationhelper.checklist.btn.tooltip.context"));
		removeExistingActionListenersOfBtnSettings();
		btnSettings.addActionListener(e -> {
			settingsBtnPressed();
			createContextAndShowContextPropertiesDialog(helperDialog.getTarget().getStartNode());
		});
	}

	private void updateStatusIconForChecklistItems() {
		checklistStatusMap.forEach((checkListLabel, status) -> checkListLabel.setIcon(resolveStatusIcon(status)));
	}
	...
	private void setHintPanelToMatchBadAuthenticationMethod() {
		labelHint.setText(Constant.messages.getString("authenticationhelper.checklist.hint.authenticationmethod"));
		btnSettings.setEnabled(true);
		btnSettings.setToolTipText(Constant.messages.getString("authenticationhelper.checklist.hint.authenticationmethod"));
		removeExistingActionListenersOfBtnSettings();
		btnSettings.addActionListener(e -> {
			settingsBtnPressed();
	...
	private void setHintPanelToMatchBadUser() {
		...
		btnSettings.setEnabled(true);
		btnSettings.setToolTipText(Constant.messages.getString("authenticationhelper.checklist.btn.tooltip.user"));
		removeExistingActionListenersOfBtnSettings();
		btnSettings.addActionListener(e -> {
			settingsBtnPressed();
	...
	private void setHintPanelToMatchIndicatorStatus(ConfigurationStatus indicatorStatus) {
		...
		case MINIMAL:
			hintText = Constant.messages.getString("authenticationhelper.checklist.hint.minimal");
			break;
		case BAD:
			hintText = Constant.messages.getString("authenticationhelper.checklist.hint.indicator");
			break;
		case GOOD:
	...
	private void resolveStausAndUpdateMap(JLabel labelChecklistItem, boolean isBadConfiguration) {
		ConfigurationStatus status = isBadConfiguration ? ConfigurationStatus.BAD : ConfigurationStatus.GOOD;
	...
	private void settingsBtnPressed() {
		...
		labelHint.setText(Constant.messages.getString("authenticationhelper.checklist.hint.refresh"));
	}
	...
	private void setStatusToNotValidatedYet() {
		...
		checklistStatusMap.forEach((k, v) -> {
	...
	private JButton getBtnSettings() {
		if (btnSettings == null) {
			...
			btnSettings.setIcon(DisplayUtils.getScaledIcon(new ImageIcon(AuthenticationConfigurationChecklistPanel.class
					.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/help/contents/images/gear.png"))));

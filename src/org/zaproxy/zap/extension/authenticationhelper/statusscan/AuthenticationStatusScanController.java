...
public class AuthenticationStatusScanController implements ScanController<AuthenticationStatusScanner> {

	private ExtensionAuthenticationHelper extAuthHelper;

	/**
	 * The {@code Lock} for exclusive access of instance variables related to
	 * multiple authentication status scans.
	 * 
	 ...
	 * <p>
	 * <strong>Note:</strong> All accesses (both write and read) should be done
	 * while holding the {@code Lock} {@code authenticationStatusScansLock}.
	 * </p>
	 * 
	...
	/**
	 * A map that contains all {@code AuthenticationStatusScan}s created (and not
	 * yet removed). Used to control (i.e. pause/resume and stop) the multiple
	 * authentication status scans and get its results. The instance variable is
	 * never {@code null}. The map key is the ID of the scan.
	 * <p>
	 * <strong>Note:</strong> All accesses (both write and read) should be done
	 * while holding the {@code Lock} {@code authenticationStatusScansLock}.
	 * </p>
	...
	public int startScan(String displayName, Target target, User user, Object[] contextSpecificObjects) {
		...
		try {
			...
			AuthenticationStatusScanner authenticationStatusScan = new AuthenticationStatusScanner(extAuthHelper, displayName,
					target, user, id, extAuthHelper.getModel().getOptionsParam().getConnectionParam());

			if (contextSpecificObjects != null) {
				for (Object obj : contextSpecificObjects) {
					if (obj instanceof OptionsParamAuthenticationHelper) {
						authenticationStatusScan.setOptionsParam((OptionsParamAuthenticationHelper) obj);
					} else if (obj instanceof AuthenticationMethod) {
						AuthenticationMethod authenticationMethod = (AuthenticationMethod) obj;
						authenticationStatusScan
								.setLoggedInIndicatorPattern(authenticationMethod.getLoggedInIndicatorPattern());
						authenticationStatusScan
								.setLoggedOutIndicatorPattern(authenticationMethod.getLoggedOutIndicatorPattern());
					}
				}
			}
	...
	public AuthenticationStatusScanner removeScan(int id) {
		...
		try {
			...
			AuthenticationStatusScanner authenticationStatusScan = this.authenticationStatusScanMap.get(id);
	...
	public int removeAllScans() {
		...
		try {
			int count = 0;
			for (Iterator<AuthenticationStatusScanner> it = authenticationStatusScanMap.values().iterator(); it
					.hasNext();) {

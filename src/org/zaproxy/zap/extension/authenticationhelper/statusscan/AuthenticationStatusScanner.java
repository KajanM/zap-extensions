...
import org.zaproxy.zap.extension.authenticationhelper.ui.AuthenticationHelperDialog;
...
public class AuthenticationStatusScanner implements GenericScanner2 {
	 ...
	 * {@code NOT_STARTED}, {@code RUNNING}, {@code PAUSED}, {@code FINISHED} or
	 * {@code STOPPED}.
	 */
	...
	/**
	 * The {@link Target} of the scan, never null.
	 * Initially set via the {@link AuthenticationHelperDialog}.
	 */
	private final Target target;
	
	/**
	 * The selected {@link User}, never null.
	 * Initially set via the {@link AuthenticationHelperDialog}.
	 */
	...
	public AuthenticationStatusScanner(ExtensionAuthenticationHelper extAuthHelper, String displayName, Target target,
			User scanUser, int scanId, ConnectionParam connectionParam) {
		...
		try {
			if (target.getStartNodes() != null) {
				...
				if (tasksTotalCount == 0) {
					authenticationStatusListener.updateProgress(scanId, getDisplayName(), 100, 0, 0, 0, 0);
					complete();
				}
	...
	private void scanAndNotifyResult(HistoryReference historyReference) {
		try {
			...
			if (msg == null)
				return; // failure result already sent
			...
			notifyNewResult(historyReference, getIndicatorStatus(loggedInIndicatorPattern, msg),
	...
	private IndicatorStatus getIndicatorStatus(Pattern indicatorPattern,
			HttpMessage msg) {
		String body = msg.getResponseBody().toString();
		String header = msg.getResponseHeader().toString();

		if (indicatorPattern == null)
			return IndicatorStatus.NOT_DEFINED;
	...
	private void notifyFailure(HistoryReference historyReference) {
		notifyNewResult(historyReference, IndicatorStatus.COULD_NOT_VERIFY,
	...
	private void postTaskExecution() {
		...
		if (progress != percentageComplete) {
			...
			authenticationStatusListener.updateProgress(scanId, getDisplayName(), percentageComplete,
					getSuccessfulAuthenticationCount(), getFailedAuthenticationCount(),
					getConflictingAuthenticationCount(), getUnknownAuthenticationCount());
		}
	...
	public void notifyNewResult(HistoryReference historyReference,
		...
		if (View.isInitialised()) {
			addMessageToAuthenticationStatusTableModel(historyReference, authenticationStatus, loggedInIndicatorStatus,
					loggedOutIndicatorStatus);
		}
	}

	private void updateAuthenticationStatusCounts(
	...
	// TODO: write relevant test cases
	private AuthenticationStatus getAuthenticationStatus(
		...
		boolean inNotDefined = loggedInIndicatorStatus
				.equals(IndicatorStatus.NOT_DEFINED);
		...
		boolean outNotDefined = loggedOutIndicatorStatus
				.equals(IndicatorStatus.NOT_DEFINED);

		if (inFound && (outNotFound || outNotDefined))
		...
		// in -> not defined
		// out -> not defined
		throw new IllegalArgumentException("atleast one of logged in or logged out indicator should be defined");
	}

	private void addMessageToAuthenticationStatusTableModel(final HistoryReference historyReference,
			final AuthenticationStatus authenticationStatus,
			final IndicatorStatus loggedInIndicatorStatus,
			final IndicatorStatus loggedOutIndicatorStatus) {

		EventQueue.invokeLater(() -> {
			authenticationStatusTableModel.addEntry(new AuthenticationStatusTableEntry(historyReference,
					authenticationStatus, loggedInIndicatorStatus, loggedOutIndicatorStatus));
		});
	}

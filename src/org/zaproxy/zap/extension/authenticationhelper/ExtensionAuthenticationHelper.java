...
import org.zaproxy.zap.extension.authenticationhelper.ui.AuthenticationHelperOptionsPanel;
import org.zaproxy.zap.extension.authenticationhelper.ui.AuthenticationStatusPanel;
import org.zaproxy.zap.extension.authenticationhelper.ui.PopupMenuItemCheckAuthentication;
...
/**
 * An extension for automated authentication configuration, guided authentication and authentication
 * status scanning.
 * 
 * @since 2.7.0
 */
public class ExtensionAuthenticationHelper extends ExtensionAdaptor
	...
	public void showCheckAuthenticationDialog(Target target) {
		...
		authenticationHelperDialog.setAppropriateToolTipAndActionListener();
	...
	private ZapMenuItem getMenuItemCheckAuthentication() {
		if (toolMenuItemCheckAuthentication == null) {
			toolMenuItemCheckAuthentication = new ZapMenuItem("authenticationhelper.topmenu.tools.title",
					KeyStroke.getKeyStroke(KeyEvent.VK_F3, 0));

			toolMenuItemCheckAuthentication.addActionListener(e -> {
	...
	public Icon getIcon() {
		if (authenticationIcon == null) {
			authenticationIcon = new ImageIcon(
					ExtensionAuthenticationHelper.class.getResource("authenticationhelper/key.png"));
		}
	...
	public List<String> getActiveActions() {
		List<AuthenticationStatusScanner> activeAuthenticationStatusScans = scanController.getActiveScans();
		if (activeAuthenticationStatusScans.isEmpty()) {
			return null;
		}

		String authenticationStatusActionPrefix = Constant.messages
				.getString("authenticationhelper.activeActionPrefix");
		List<String> activeActions = new ArrayList<>(activeAuthenticationStatusScans.size());
		for (AuthenticationStatusScanner activeAuthenticationStatusScan : activeAuthenticationStatusScans) {
			activeActions.add(MessageFormat.format(authenticationStatusActionPrefix,

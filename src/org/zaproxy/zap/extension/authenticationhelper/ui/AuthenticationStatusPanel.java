...
public class AuthenticationStatusPanel
	...
	public static final String HTTP_MESSAGE_CONTAINER_NAME = "AuthenticationStatusScanMessageContainer";

	private static final String ZERO_REQUESTS_LABEL_TEXT = "0";

	private static final AuthenticationStatusTableModel EMPTY_MESSAGES_TABLE_MODEL = new AuthenticationStatusTableModel();
	...
	public AuthenticationStatusPanel(ExtensionAuthenticationHelper extensionAuthenticationHelper) {
		super("authenticationhelper",
				new ImageIcon(ExtensionAuthenticationHelper.class
						.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/key.png")),
				extensionAuthenticationHelper);
		this.extensionAuthenticationHelper = extensionAuthenticationHelper;

		setDefaultAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask() | KeyEvent.SHIFT_DOWN_MASK, false));
	...
	private JLabel getSuccessfulAuthenticationCountNameLabel() {
		if (successfullAuthenticationCountNameLabel == null) {
			...
			successfullAuthenticationCountNameLabel.setText(
					Constant.messages.getString("authenticationhelper.toolbar.successsfulAuthenticationCount.label"));
		}
	...
	private JLabel getFailedAuthenticationCountNameLabel() {
		if (failedAuthenticationCountNameLabel == null) {
			...
			failedAuthenticationCountNameLabel.setText(
					Constant.messages.getString("authenticationhelper.toolbar.failedAuthenticationCount.label"));
		}
	...
	private JLabel getConflictingAuthenticationCountNameLabel() {
		if (conflictAuthenticationCountNameLabel == null) {
			...
			conflictAuthenticationCountNameLabel.setText(
					Constant.messages.getString("authenticationhelper.toolbar.conflictingAuthenticationCount.label"));
		}
	...
	private JLabel getUnknownAuthenticationCountNameLabel() {
		if (unknownAuthenticationCountNameLabel == null) {
			...
			unknownAuthenticationCountNameLabel.setText(
					Constant.messages.getString("authenticationhelper.toolbar.unknownAuthenticationCount.label"));
		}
	...
	public JButton getNewScanButton() {
		if (scanButton == null) {
			scanButton = new JButton(Constant.messages.getString("authenticationhelper.toolbar.button.check"));
			scanButton.setIcon(DisplayUtils.getScaledIcon(new ImageIcon(AuthenticationStatusPanel.class
					.getResource("/org/zaproxy/zap/extension/authenticationhelper/resources/key.png"))));
			scanButton.addActionListener(
					e -> extensionAuthenticationHelper.showCheckAuthenticationDialog(getSiteTreeTarget()));
		}
		return scanButton;
	}

	private SiteNode getSiteTreeTarget() {
		if (!extensionAuthenticationHelper.getView().getSiteTreePanel().getTreeSite().isSelectionEmpty()) {
			return (SiteNode) extensionAuthenticationHelper.getView().getSiteTreePanel().getTreeSite()
	...
	@Override
	public void updateProgress(int id, String host, int progress, int successCount, int failedCount,
			int conflictingCount, int unknownCount) {
